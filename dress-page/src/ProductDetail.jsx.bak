    ...
    return (
      <div className="min-h-screen bg-gray-50">
        ...
        <div className="max-w-4xl mx-auto p-6 text-center">
          ...
          <Link to="/collection" className="mt-4 inline-block bg-pink-600 text-white px-4 py-2 rounded">Browse Collection</Link>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="max-w-7xl mx-auto px-4 py-8">
        ...
        <div className="text-sm text-gray-500 mb-6">
          <Link to="/collection" className="hover:text-pink-600">Home</Link> / 
        ...
        <div className="grid md:grid-cols-3 gap-8">
          ...
          <div className="md:col-span-1">
            <div className="bg-white rounded-lg shadow p-6 sticky top-20">
              ...
              {product.tag && (
                <div className="mt-4 bg-pink-100 text-pink-700 px-4 py-2 rounded-lg text-center font-semibold text-sm">
                  {product.tag}
                </div>
              )}
            </div>
            ...
            <div className="bg-white rounded-lg shadow p-6">
              ...
              <div className="mb-6 pb-6 border-b">
                <div className="flex items-baseline gap-3 mb-2">
                  <span className="text-3xl font-bold text-pink-600">₹{getProductPrice()}</span>
                  {product.mrp && (
                    <>
                      ...
                      {[
                        ...
                        <button
                          ...
                          className={`w-10 h-10 rounded-full border-2 transition ${
                            color === c.name ? 'ring-2 ring-pink-600 border-pink-600 scale-110' : 'border-gray-300'
                          } ${c.color}`}
                          title={c.name}
                        >
              ...
              <div className="flex gap-4 mb-6">
                <button
                  ...
                  className="flex-1 bg-pink-600 text-white px-6 py-3 rounded-lg hover:bg-pink-700 font-semibold transition"
                >
                  Add to Cart
                </button>
                <button
              ...
              <div className="grid grid-cols-3 gap-4 mb-6">
                ...
                <div className="flex flex-col items-center gap-2 p-3 bg-gray-50 rounded-lg">
                  <RefreshCw size={24} className="text-pink-600" />
                    ...
                    <div>
                      ...
                      <div className="overflow-x-auto">
                        <table className="w-full text-sm border-collapse">
                          <thead>
                            <tr className="bg-pink-50 border-b-2 border-pink-600">
                          ...
                          <tbody>
                            {Object.entries(product.sizeGuide).map(([sizeKey, measurements], idx) => (
                              ...
                              >
                                <td className={`px-4 py-3 font-bold ${size === sizeKey || sizeKey === size ? 'text-pink-600' : 'text-gray-900'}`}>
                                  {sizeKey}
                                </td>
                      ...
                      <div className="mt-8 grid grid-cols-1 md:grid-cols-2 gap-4">
                        {Object.entries(product.sizeGuide).map(([sizeKey, measurements]) => (
                          ...
                          >
                            <div className={`font-bold text-lg mb-1 ${size === sizeKey || sizeKey === size ? 'text-pink-600' : 'text-gray-900'}`}>
                              {sizeKey}
                            </div>
                    ...
                    <div>
                          ...
                          <tbody>
                            {Object.entries(product.quantityGuide).map(([qtyKey, qtyData], idx) => {
                              ...
                              return (
                                ...
                                >
                                  <td className={`px-4 py-3 font-bold ${selectedQuantity === qtyKey ? 'text-pink-600' : 'text-gray-900'}`}>
                                    {qtyKey}
                                  </td>
                                  <td className="px-4 py-3 font-semibold text-pink-600">₹{qtyData.price}</td>
                      ...
                      <div className="mt-8 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        {Object.entries(product.quantityGuide).map(([qtyKey, qtyData]) => (
                          ...
                          >
                            <div className={`font-bold text-lg mb-2 ${selectedQuantity === qtyKey ? 'text-pink-600' : 'text-gray-900'}`}>
                              {qtyKey}
                            </div>
                            <div className="text-2xl font-bold text-pink-600 mb-2">₹{qtyData.price}</div>
                  ...
                  <div className="space-y-4">
                    <div className="flex items-center gap-4 mb-6">
                      ...
                      <div className="flex-1">
                        ...
                        <button className="mt-3 text-pink-600 font-semibold hover:underline text-sm">Write a Review</button>
                      </div>
                    </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                  {location.state.related.slice(0, 4).map((r) => (
                    <div key={r.id} className="bg-gray-50 rounded-lg p-4 hover:shadow-lg transition">
                      ...
                      >
                        <p className="text-sm font-medium line-clamp-2 group-hover:text-pink-600 mb-2">{r.name}</p>
                        <div className="flex items-center gap-2 mb-3">
                          <p className="text-pink-600 font-bold text-sm">₹{r.price}</p>
                        ...
                        <div className="mb-3">
                          ...
                          <div className="flex gap-1 flex-wrap">
                            {Object.keys(r.sizeGuide).slice(0, 3).map((s) => (
                              <button
                                key={s}
                                className="px-2 py-1 text-xs border border-gray-300 rounded hover:bg-pink-100 hover:border-pink-600 transition"
                              >
                                {s}
                              </button>
                            ))}
                            {Object.keys(r.sizeGuide).length > 3 && (
                              <button className="px-2 py-1 text-xs border border-gray-300 rounded hover:bg-pink-100 hover:border-pink-600 transition">
                                +{Object.keys(r.sizeGuide).length - 3}
                              </button>
                            )}
                          </div>
                      ...
                      <Link
                        to={`/product/${r.id}`}
                        state={{ product: r }}
                        className="w-full bg-pink-600 text-white px-3 py-2 rounded text-sm font-semibold hover:bg-pink-700 transition block text-center"
                      >
                        View Details
                      </Link>
                    </div>
